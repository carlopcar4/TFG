...
class Alcorque(models.Model):
    ...
    barrio = models.ForeignKey("inventario.Barrio", on_delete=models.PROTECT, related_name="alcorques",)
    estado = models.CharField(max_length=50, choices=Estado.choices, default=Estado.BUENO)
    potencial_plant = models.CharField(max_length=10, choices=Potencial.choices, default=Potencial.APTO)
    latitud = models.DecimalField(max_digits=9, decimal_places=6, null=True, blank=True)
    longitud = models.DecimalField(max_digits=9, decimal_places=6, null=True, blank=True)
    direccion = models.TextField(null=True, blank=True)
    observaciones = models.TextField(null=True, blank=True)
